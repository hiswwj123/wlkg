...
public class SmsUtils {
    ...
    public CommonResponse sendSms(String phone, String code) {
        ...
        //可选:模板中的变量替换 JSON 串,如模板内容为"亲爱的${name},您的验证码为
        // ${code}"时,此处的值为
        request.putQueryParameter("TemplateParam", "{\"code\":\"" + code +
                "\"}");
        try {
            CommonResponse response = client.getCommonResponse(request);
            logger.info("发送短信结果为：" + response.getData());
            //{"Message":"OK","RequestId":"08C3FC82-159B-4FCC-A32C-4E1638787F 09 "," BizId ":" 351910169162041803 ^ 0 "," Code ":" OK "}
            return response;
